...
from nengo_ocl.clraggedarray import CLRaggedArray, data_ptr, to_device
...
def plan_presentinput(queue, Y, t, signals, dt, pres_t=None, tag=None):
    ...
    gsize = (max_len, N)
    lsize = (max_len, 1)
    rval = Plan(queue, _fn, gsize, lsize=lsize, name="cl_presentinput", tag=tag)
    rval.full_args = full_args     # prevent garbage-collection
    return rval
...
% if transposed:
                __global const ${type} *fcij = &f[(c*${sij} + ii*${sj} + jj)*${nf}];
% else:
